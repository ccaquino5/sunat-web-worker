...
app.post("/consultar-sunat", async (req, res) => {
  ...
  try {
    ...
    await page.fill('input[name*="numRuc"], input[id*="ruc"], input[name*="ruc"]', String(ruc_proveedor));
    await page.selectOption('select[name*="codComp"], select[id*="codComp"], select[name*="tipo"]', String(sunat_tipo_comprobante_web));
    await page.fill('input[name*="numSerie"], input[id*="serie"], input[name*="serie"]', String(sunat_serie_web).trim());
    await page.fill('input[name*="numCpe"], input[id*="numero"], input[name*="numero"]', String(sunat_numero_web).trim());
    await page.fill('input[name*="fecEmision"], input[id*="fecha"], input[name*="fecha"]', fecha);
    await page.fill('input[name*="mtoImporte"], input[id*="importe"], input[name*="importe"]', importe);

    await Promise.all([
      page.click('button[type="submit"], input[type="submit"], button:has-text("Consultar"), button:has-text("Buscar")'),
    ...
    let detalle = bodyText.slice(0, 2000);
